...
import { createZone, getZonesByTenant, updateZone, deleteZone, } from './zones.service.js';
/**
 * Maneja la creación de una nueva zona.
 */
export const createZoneHandler = async (req, res, next) => {
    try {
 ...
 */
export const getZonesByTenantHandler = async (req, res, next) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ success: false, message: 'Acción no permitida.' });
 ...
 */
export const updateZoneHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        const tenantId = req.user?.tenantId;
        if (!id || !tenantId) {
 ...
 */
export const deleteZoneHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        const tenantId = req.user?.tenantId;
        if (!id || !tenantId) {

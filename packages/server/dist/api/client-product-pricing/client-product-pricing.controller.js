...
import { createPricingRule, getPricingRulesByClient, updatePricingRule, deletePricingRule, } from './client-product-pricing.service.js';
/**
 * Maneja la creación de una nueva regla de precios.
 */
export const createPricingRuleHandler = async (req, res, next) => {
    try {
 ...
 */
export const getPricingRulesByClientHandler = async (req, res, next) => {
    try {
 ...
 */
export const updatePricingRuleHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        if (!id) {
 ...
 */
export const deletePricingRuleHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({ success: false, message: 'El ID de la regla es requerido.' });

 ...
 */
export const getConsumptionReportHandler = async (req, res, next) => {
    try {
 ...
 */
export const getProductConsumptionDetailHandler = async (req, res, next) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ success: false, message: 'Acción no permitida. Tenant no identificado.' });
 ...
 */
export const getInvoicingReportHandler = async (req, res, next) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ success: false, message: 'Acción no permitida. Tenant no identificado.' });

...
import { createRouteTemplate, getRouteTemplatesByTenant, getRouteTemplateById, updateRouteTemplate, deleteRouteTemplate, } from './route-templates.service.js';
/**
 * Maneja la creación de una nueva Ruta Maestra.
 */
export const createRouteTemplateHandler = async (req, res, next) => {
    try {
 ...
 */
export const getRouteTemplatesHandler = async (req, res, next) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ success: false, message: 'Acción no permitida.' });
 ...
 */
export const getRouteTemplateByIdHandler = async (req, res, next) => {
    try {
        const tenantId = req.user?.tenantId;
        const { id } = req.params;
        if (!tenantId || !id) {
 ...
 */
export const updateRouteTemplateHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        const tenantId = req.user?.tenantId;
        if (!id || !tenantId) {
 ...
 */
export const deleteRouteTemplateHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        const tenantId = req.user?.tenantId;
        if (!id || !tenantId) {

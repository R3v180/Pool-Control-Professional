...
import { getAssignableUsersByTenant, updateUserAvailability, getAvailabilitiesForUser, setUserAvailability, } from './users.service.js';
export const getAssignableUsersHandler = async (req, res, next) => {
    try {
...
export const updateUserAvailabilityHandler = async (req, res, next) => {
    try {
...
export const getUserAvailabilitiesHandler = async (req, res, next) => {
    try {
        const { id: userId } = req.params;
        const tenantId = req.user?.tenantId;
        if (!userId || !tenantId) {
...
export const setUserAvailabilityHandler = async (req, res, next) => {
    try {
        const tenantId = req.user?.tenantId;

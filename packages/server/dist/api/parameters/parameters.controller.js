...
import { createParameterTemplate, deleteParameterTemplate, getParameterTemplatesByTenant, updateParameterTemplate, } from './parameters.service.js';
/**
 * Maneja la creación de una nueva plantilla de parámetro.
 */
export const createParameterTemplateHandler = async (req, res, next) => {
    try {
 ...
 */
export const getParameterTemplatesByTenantHandler = async (req, res, next) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ success: false, message: 'Acción no permitida.' });
 ...
 */
export const updateParameterTemplateHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        const tenantId = req.user?.tenantId;
        if (!id || !tenantId) {
 ...
 */
export const deleteParameterTemplateHandler = async (req, res, next) => {
    try {
        const { id } = req.params;
        const tenantId = req.user?.tenantId;
        if (!id || !tenantId) {
